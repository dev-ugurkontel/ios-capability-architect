import { findRecord, loadRegistry, searchRecords, deduplicateDocumentation } from "./registry.js";
import type {
  CapabilityMatch,
  CapabilityRecord,
  IdeaAnalysis,
...
const requirementRules: Array<{
  ...
  { kind: "background", description: "Continue or resume work while the app is not foregrounded", terms: ["background", "arka plan", "continuous", "sürekli"] },
  ...
  { kind: "ai_ml", description: "Use machine learning or generative AI", terms: ["ai", "artificial intelligence", "machine learning", "model", "yapay zekâ", "yapay zeka", "ml"] },
  { kind: "monetization", description: "Monetize through App Store purchases or subscriptions", terms: ["subscription", "purchase", "monetization", "abonelik", "satın alma", "ücret"] },
  ...
  { kind: "privacy", description: "Handle sensitive personal data with elevated privacy controls", terms: ["health", "location", "child", "finance", "identity", "biometric", "sağlık", "konum", "çocuk", "finans", "kimlik", "biyometr"] }
];
...
export function analyzeAppIdea(input: {
  ...
  if (input.on_device_priority !== "neutral" && !requirements.some((requirement) => requirement.kind === "on_device")) {
    requirements.push({
      id: "req-on_device",
      kind: "on_device",
      description: input.on_device_priority === "required" ? "Processing must remain on device" : "Prefer on-device processing where practical",
  ...
  const assumptions = [
    ...
    input.minimum_os_version ? `The deployment target is ${input.minimum_os_version}.` : "The deployment target is flexible and should prefer the current stable SDK."
  ];
  const constraints = [
...
function scoreRecord(requirement: Requirement, record: CapabilityRecord): { score: number; reasons: string[] } {
  const haystack = [record.id, record.name, record.summary, ...record.aliases, ...record.keywords, ...record.supported_use_cases]
    .join(" ").toLocaleLowerCase("en-US");
  const tokens = [...requirement.keywords, ...requirement.description.toLocaleLowerCase("en-US").split(/[^\p{L}\p{N}-]+/u)]
...
export async function resolveCapabilities(input: {
  ...
  const records = (await loadRegistry()).filter((record) => input.include_beta || record.stable_or_beta !== "beta");
  const matches = input.requirements.flatMap((requirement) => records
    .map((record) => ({ record, ...scoreRecord(requirement, record) }))
    .filter(({ score }) => score > 0)
    .sort((left, right) => right.score - left.score)
    .slice(0, input.maximum_results_per_requirement)
    .map(({ record, score, reasons }) => ({ requirement_id: requirement.id, capability_id: record.id, score, reasons, record })));
...
export async function compareImplementationOptions(capabilityIds: string[], criteria: string[]): Promise<ToolEnvelope<{ criteria: string[]; options: Array<Record<string, unknown>> }>> {
...
export async function checkAvailability(input: {
  ...
  const results = records.map((record) => {
    ...
    if (!record.platforms.includes(input.platform)) reasons.push(`${input.platform} is not listed as supported.`);
    if (record.stable_or_beta === "beta" && !input.allow_beta) reasons.push("This record is beta and beta use was not allowed.");
    if (requestedMajor !== undefined && minimumMajor !== undefined && requestedMajor < minimumMajor) reasons.push(`Requires ${input.platform} ${minimum} or later.`);
    if (record.hardware_requirements.length > 0 && !input.device) reasons.push("Runtime hardware eligibility must be checked.");
    if (record.region_restrictions.length > 0 && !input.region) reasons.push("Runtime region availability must be checked.");
    if (record.language_restrictions.length > 0 && !input.language) reasons.push("Runtime language availability must be checked.");
    return {
...
export async function auditPermissionsAndEntitlements(capabilityIds: string[]): Promise<ToolEnvelope<Record<string, string[]>>> {
  const records = await resolveIds(capabilityIds);
  return envelope({
...
export async function generateArchitecture(idea: string, capabilityIds: string[], projectScale: string): Promise<ToolEnvelope<Record<string, unknown>>> {
  const records = await resolveIds(capabilityIds);
  ...
  const components = [
    ...
    { layer: "Networking", recommendation: records.some((record) => record.on_device_level === "cloud_required" || record.on_device_level === "hybrid") ? "Use a narrow URLSession client only for requirements that cannot remain local; make offline state explicit." : "No server by default." },
    ...
    { layer: "Background execution", recommendation: records.some((record) => record.background_modes.length > 0 || record.category === "background_execution") ? "Event-driven, resumable jobs with expiration handling; never promise exact timing." : "Foreground-only unless a verified capability is added." },
...
export async function generateImplementationPlan(capabilityIds: string[], includeCodeSpike: boolean): Promise<ToolEnvelope<{ phases: Array<Record<string, unknown>> }>> {
  const records = await resolveIds(capabilityIds);
  const phases = [
    ...
    { id: "framework-integration", depends_on: ["mvp-foundation"], goal: "Integrate selected Apple frameworks one at a time.", deliverables: records.map((record) => `${record.name} adapter and integration tests`) },
    ...
    { id: "background-and-extensions", depends_on: ["permissions"], goal: "Add only verified background modes and extension targets.", deliverables: unique(records.flatMap((record) => [...record.background_modes, ...record.related_extensions])) },
...
export async function searchOfficialAppleDocs(query: string, capabilityIds: string[], maximumResults: number): Promise<ToolEnvelope<{ results: Array<Record<string, string>> }>> {
  ...
  const references = deduplicateDocumentation(records)
    .map((reference) => ({ reference, score: queryTokens.filter((token) => `${reference.title} ${reference.url}`.toLocaleLowerCase("en-US").includes(token)).length }))
    .sort((left, right) => right.score - left.score)
    .slice(0, maximumResults)
    .map(({ reference }) => ({ title: reference.title, url: reference.url, source_type: reference.source_type, verified_at: reference.verified_at }));
...
export async function refreshCapabilityRegistry(dryRun: boolean, sourceUrls: string[]): Promise<ToolEnvelope<Record<string, unknown>>> {
