...
const documentationReferenceSchema = z.object({
  ...
  url: z.url().refine((url) => url.startsWith("https://developer.apple.com/") || url.startsWith("https://developers.openai.com/"), {
...
export const resolveCapabilitiesInputSchema = z.object({
  requirements: z.array(z.object({
    id: z.string(),
    kind: z.enum([
      "product_goal", "data", "hardware", "background", "notification", "on_device",
      "networking", "ai_ml", "privacy", "monetization", "platform"
    ]),
...
export const compareOptionsInputSchema = z.object({
  ...
  criteria: z.array(z.enum([
    "complexity", "minimum_os", "on_device", "privacy", "performance", "energy",
    "hardware", "entitlement", "app_review", "maintenance", "testability"
